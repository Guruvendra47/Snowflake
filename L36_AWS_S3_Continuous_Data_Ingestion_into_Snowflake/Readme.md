...
This document provides a complete step-by-step procedure. If you need clarification on any section, please refer to the AWS_Snowflake_Continous_Data_Loading_Step_By_Step_Procedure.pdf file uploaded in the folder.
...
2. (Optional) we Check **Require external ID** (best practice when 3rd party resume this role).
...
3. Replace the Value under **AWS** with the **`STORAGE_AWS_IAM_USER_ARN`** value which we got from snowflakes and **sts:ExternalId** with **`STORAGE_AWS_EXTERNAL_ID`**.
